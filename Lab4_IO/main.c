...
int main(void){    
  ...
  while(1){
    SW1 = GPIO_PORTF_DATA_R&0x01;     // read PF4 into SW1 (Initially had 0x10 which was wrong for SW1)
    SW2 = GPIO_PORTF_DATA_R&0x10;     // read PF0 into SW2 (Initially had 0x01 which was wrong for SW2)
    if(SW1&&SW2){                     // neither presed
      GPIO_PORTF_DATA_R = 0x00;       // LED is off
    } else{                           
      if(SW1&&(!SW2)){                // SW1 pressed
        GPIO_PORTF_DATA_R = 0x02;     // LED is RED
      } else{                        
        if((!SW1)&&SW2){              // SW2 pressed
